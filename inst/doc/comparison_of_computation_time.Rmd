...
First we check, if all calculation approaches give the same results (lmomco::lmoms is added as comparison). 
```{r}
...
In `TLMoments` the *recursive approach* is the fastest. After this, the *pwm approach* is to be prefered over the *direct approach*. 
...
Therefore these are the defaults in this package, but the other computation methods (*direct* and *pwm*) are still available. 
